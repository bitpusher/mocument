...
{
    public static class HttpArchiveJsonExport
    {
        internal static readonly int iMaxBinaryBodyLength = FiddlerApplication.Prefs.GetInt32Pref("fiddler.importexport.HTTPArchiveJSON.MaxBinaryBodyLength", 0x8000);
        ...
        private static Hashtable getBodyInfo(Session oS)
        {
            ...
            if (((flag && ("text/plain" == mIMEType)) && (oS.responseBodyBytes.Length > 3)) &&
            ...
            if (oS.responseBodyBytes.Length < iMaxBinaryBodyLength)
            {
        ...
        private static Hashtable getResponse(Session oS)
        {
            ...
            hashtable.Add("redirectURL", oS.oResponse["Location"]);
            hashtable.Add("bodySize", oS.responseBodyBytes.Length);
            hashtable.Add("headers", getHeadersAsArrayList(oS.oResponse.headers));
            hashtable.Add("cookies", getCookies(oS.oResponse.headers));
            hashtable.Add("content", getBodyInfo(oS));
            return hashtable;
        }
        ...
        public static string WriteStream(IEnumerable<Session> oSessions
            //, EventHandler<ProgressCallbackEventArgs> evtProgressNotifications
            )
        {
            Hashtable hashtable = new Hashtable();
            hashtable.Add("version", "1.2");
            hashtable.Add("pages", null);
            hashtable.Add("comment", "exported @ " + DateTime.Now.ToString());
            Hashtable hashtable2 = new Hashtable();
            hashtable2.Add("name", "Mocument.it");
            hashtable2.Add("version", "1.0");
            hashtable2.Add("comment", "http://www.mocument.it");
            ...
            foreach (Session session in oSessions)
            {
                ...
                //{
                //    ProgressCallbackEventArgs e = new ProgressCallbackEventArgs(
                //        ((float) num)/((float) oSessions.Length),
                //        "Wrote " + num.ToString() + " sessions to HTTPArchive.");
